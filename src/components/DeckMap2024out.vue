<template>
  <div class="tool-bar">
    ...
    <button class="download" @click="downloadHighResImage">Download HD Image</button>
  </div>
...
async function downloadHighResImage() {
  if (!deckgl) {
    console.error('DeckGL instance is not available.');
    return;
  }

  try {
    // 使用Deck.gl的exportImage方法导出高清图
    const image = await deckgl.exportImage('png', {
      quality: 1, // 最高质量
      width: deckgl.width * 2, // 提高分辨率
      height: deckgl.height * 2,
    });

    // 创建下载链接
    const link = document.createElement('a');
    link.href = image;
    link.download = 'high_res_map.png'; // 下载文件名
    link.click();
  } catch (error) {
